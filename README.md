...
This fork adds preliminary AMD GPU (ROCm) enablement on top of the upstream DeepEP:

- xGMI / Infinity Fabric intranode + InfiniBand / RoCE RDMA internode via HIPified kernels
- Unified Python API — the same `deep_ep` interface as the NVIDIA path
- Some NVIDIA-only optimizations (e.g., PTX loads) are not mirrored; the AMD path focuses on correctness and baseline overlap

**Notice:** The PyTorch commit [e4adf5d](https://github.com/pytorch/pytorch/commit/e4adf5df39d9c472c7dcbac18efde29241e238f0) fixed a bug with the `-fgpu-rdc` flag in the PyTorch ROCm submodule. Please use any PyTorch version later than this commit (e.g., nightly PyTorch). If using an older version, apply the patch described in the [rocSHMEM Installation Guide](https://github.com/ROCm/rocm-systems/blob/develop/projects/rocshmem/README.md).

### ROCm Requirements

- MI308X / MI300X / MI350X GPU
- Python 3.8 and above
- ROCm 6.3.4 or 7.2.0 and above
...
### Install rocSHMEM dependency

DeepEP (AMD) depends on [rocSHMEM](https://github.com/ROCm/rocm-systems/tree/develop/projects/rocshmem). Please refer to the [rocSHMEM Installation Guide](third-party/README.md) for instructions.

### ROCm Development and Installation

```bash
git clone https://github.com/ROCm/DeepEP
cd DeepEP

# Without MPI (make sure rocSHMEM was built with -DUSE_EXTERNAL_MPI=OFF)
# NIC_TYPE can be: cx7, thor2, io (default: cx7)
...
DeepEP (AMD) supports the following NIC families (selected via `--nic`):

| NIC flag | Driver   | Examples                        |
|----------|----------|---------------------------------|
| `cx7`    | mlx5     | mlx5_0, mlx5_1                  |
| `thor2`  | bnxt_re  | bnxt_re0 ... bnxt_re7           |
| `io`     | Pensando | AMD Pensando AI NIC             |
