...
// low latency+RocSHMEM has issue with CTX for certain NICs
...
__global__ __launch_bounds__(kNumWarpGroups * kNumWarpsPerGroup * kWarpSize, 1)
#else
...
#define DISPATCH_LAUNCH_CASE(hidden) { \
    auto dispatch_func = \
      use_fp8 \
        ? ( use_ue8m0 \
              ? ( round_scale \
                    ? ( kMultinode \
                          ? dispatch<true, true, true, kNumWarpGroups, kNumWarpsPerGroup, hidden> \
                          : dispatch<true, true, false, kNumWarpGroups, kNumWarpsPerGroup, hidden> ) \
                    : ( kMultinode \
                          ? dispatch<true, false, true, kNumWarpGroups, kNumWarpsPerGroup, hidden> \
                          : dispatch<true, false, false, kNumWarpGroups, kNumWarpsPerGroup, hidden> ) ) \
              : ( kMultinode \
                    ? dispatch<true, false, true, kNumWarpGroups, kNumWarpsPerGroup, hidden> \
                    : dispatch<true, false, false, kNumWarpGroups, kNumWarpsPerGroup, hidden> ) ) \
        : ( kMultinode \
              ? dispatch<false, false, true, kNumWarpGroups, kNumWarpsPerGroup, hidden> \
              : dispatch<false, false, false, kNumWarpGroups, kNumWarpsPerGroup, hidden> ); \
    LAUNCH_KERNEL_NON_COOPERATIVE(&cfg, dispatch_func, \
                  ...
                  num_topk, num_experts, rank, num_ranks, phases, round_scale); \
    } break
...
// ============================================================================
// LogFMT encoding/decoding functions
// ============================================================================
#ifndef USE_ROCM
...
void combine(void* combined_x,
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        for (int token_idx = offset + sub_warp_id; token_idx < offset + num_tokens_to_send; token_idx += kNumWarpsPerGroup) {
            ...
            const auto dst_ptr = reinterpret_cast<uint64_t>(rdma_recv_x) +
                (global_expert_idx * num_max_dispatch_tokens_per_rank + src_idx) * num_bytes_per_slot + sizeof(int4);
...
#else
        // TMA-based send with optional LogFMT encoding
...
#ifdef USE_ROCM
    // ROCm: Simple reduction without TMA
...
#else
    // TMA-based reduction with optional LogFMT decoding
...
void combine(void* combined_x,
    ...
    // LogFMT not supported on ROCm
    EP_HOST_ASSERT(!use_logfmt && "LogFMT not supported on ROCm");

    bool kMultinode = (num_ranks > 8);

#define COMBINE_LAUNCH_CASE(hidden) { \
    auto combine_func = kMultinode \
        ? combine<true, kNumWarpGroups, kNumWarpsPerGroup, hidden, kNumMaxTopk> \
...
// ============================================================================
// Utility functions: query/update/clean mask buffer
// ============================================================================
template <int kNumThreads>
